...
typedef struct {
    ...
    XOuter_product_uio_map maps[ MAX_UIO_MAPS ];
} XOuter_product_uio_info;

/***************** Variable Definitions **************************************/
static XOuter_product_uio_info uio_info;
...
static int uio_info_read_name(XOuter_product_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOuter_product_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOuter_product_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOuter_product_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOuter_product_Initialize(XOuter_product *InstancePtr, const char* InstanceName) {
	XOuter_product_uio_info *InfoPtr = &uio_info;
