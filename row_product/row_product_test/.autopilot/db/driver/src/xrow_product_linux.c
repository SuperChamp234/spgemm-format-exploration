...
typedef struct {
    ...
    XRow_product_uio_map maps[ MAX_UIO_MAPS ];
} XRow_product_uio_info;

/***************** Variable Definitions **************************************/
static XRow_product_uio_info uio_info;
...
static int uio_info_read_name(XRow_product_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRow_product_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRow_product_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRow_product_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRow_product_Initialize(XRow_product *InstancePtr, const char* InstanceName) {
	XRow_product_uio_info *InfoPtr = &uio_info;
